...
MODEL_XML = os.path.join('MPL', 'MPL_Basic.xml')


class MPLGraspEnv(mpl_env.MPLEnv):
    def __init__(
        ...
        Args:
            ...
            distance_threshold (float, in meters): the threshold + ball radius determines whether ball is off ground
    ...
    def _is_done(self):
        """
        Keeps x,y,z within bounds
        If target object moves too far, episode is done
        """
    ...
    def _is_on_ground(self):
        """
        Checks if target object is above ground
        Counts consecutive steps of target being above ground
        """
        # distance_threshold + radius of target object determines off-ground 
    ...
    def _reset_sim(self):
        ...
        # Choose randomized target object
    ...
    def _set_action(self, action):
        ...
        # Indices of motors actuated by agent
        ctrl_idx = [2, 3, 4, 12, 7]
        ...
        if self.control_mode == 'simulated':
            ...
            # simulate user grasp on digits not controlled by agent
    ...
    def _get_obs(self):
        ...
        observation = np.concatenate([palm, fingers, robot_qpos, robot_qvel])
...
class MPLGraspTrainEnv(MPLGraspEnv, utils.EzPickle):
    def __init__(self, target_position='random', target_rotation='z', reward_type='sparse'):
        ...
        MPLGraspEnv.__init__(self, 
            model_path=MODEL_XML, n_targets=30, target_body='obj_:joint',
...
class MPLGraspOpEnv(MPLGraspEnv, utils.EzPickle):
    def __init__(self, target_position='random', target_rotation='z', reward_type='sparse'):
        utils.EzPickle.__init__(self, target_position, target_rotation, reward_type)
        MPLGraspEnv.__init__(self, 
            model_path=MODEL_XML, n_targets=30, target_body='obj_:joint',
...
class MPLGraspTrackEnv(MPLGraspEnv, utils.EzPickle):
    def __init__(self, target_position='random', target_rotation='z', reward_type='sparse'):
        utils.EzPickle.__init__(self, target_position, target_rotation, reward_type)
        MPLGraspEnv.__init__(self, 
            model_path=MODEL_XML, n_targets=30, target_body='obj_:joint',
